...
class Message:
    ...
    def decodage(self):
        # Décodage du type (1 byte)
        self.type, = struct.unpack("B", self.byte[:1])
        # Décodage du port (4 bytes)
        self.port, = struct.unpack("I", self.byte[1:5])
        # Décodage de la taille (4 bytes)
        self.size, = struct.unpack("I", self.byte[5:9])
        # Décodage des données sérialisées (le reste des bytes)
        self.data = pickle.loads(self.byte[9:])
